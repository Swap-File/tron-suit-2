...
boolean Adafruit_SSD1306::readPixel(int16_t x, int16_t y) {
 ...
 if ((buffer[x+ (y/8)*SSD1306_LCDWIDTH] >> (y & 7)) & 0x01 == 0x01){
 return true;
}
   return false; 
}
...
void Adafruit_SSD1306::reinit(void) {
...
//long int start = micros();
  ...
  ssd1306_command(2); // Page start address (0 = reset)
