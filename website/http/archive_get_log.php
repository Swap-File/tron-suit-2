...
try {
	...
	$ending = $starting + 100;
    $stmt = $conn->prepare("SELECT * FROM log ORDER BY id DESC LIMIT :starting,:ending");
...
foreach ($rowarray as $row) {
	print "<tr>";  
        print "<td>" .  $row["timeStamp"]  . "</td><td bgcolor=\"" . $row["color1"] . "\">" . $row["color1"] . "</td>". "<td bgcolor=\"" . $row["color2"] . "\">" .$row["color2"] . "</td>";	
    print "</tr>\n";
}  
print "</table>\n";  
}
