...
void setup() {
	...
	if (digitalRead(10)){
		...
		//mpu.setXAccelOffset(-3230); // 1688 factory default for my test chip
		//mpu.setYAccelOffset(-1430); // 1688 factory default for my test chip
		mpu.setZAccelOffset(1665); // 1688 factory default for my test chip
	}
	else{
		gloveid = 1;
		// supply your own gyro offsets here, scaled for min sensitivity
	...
	if (size != 9 && size != 35){
		...
		if (crc != buffer[size - 1]){
			...
			if (size == 9){
				...
				uint8_t offset = 0;
				if (gloveid == 0){
					//if glove 0, pass the command on
